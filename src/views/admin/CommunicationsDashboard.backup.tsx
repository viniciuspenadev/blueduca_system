...
import { Loader2, Plus, CheckCircle2, XCircle, Send, MessageSquare, FileText, Users, Search, AlertCircle, Sparkles, Smile } from 'lucide-react';
...
const CommunicationsDashboard: React.FC = () => {
    ...
    }, [currentSchool]); // Removed selectedComm dependency to avoid recreation
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            let query = supabase
                .from('communications')
                .select(`
                    id, title, created_at, metadata, content, priority, target_type, target_ids,
            ...
            if (user?.role === 'TEACHER') {
                query = query.or(`sender_profile_id.eq.${user.id},id.in.(select communication_id from communication_recipients where student_id in (select student_id from class_enrollments where class_id in (select class_id from class_teachers where teacher_id = '${user.id}')))`);
            }
            ...
            const enriched = await Promise.all(comms.map(async (c: any) => {
                const [recipientsResult, repliesResult] = await Promise.all([
                    supabase.from('communication_recipients').select('read_at, response, guardian:profiles!guardian_id(name)', { count: 'exact' }).eq('communication_id', c.id),
                    supabase.from('communication_replies').select('guardian_id, is_admin_reply').eq('communication_id', c.id).order('created_at', { ascending: true })
                ]);

                const recipients = recipientsResult.data || [];
                const totalRecipients = recipientsResult.count || 0;
                const readCount = recipients.filter(r => r.read_at).length;
                const responses = recipients.filter(r => r.response).map(r => ({ response: r.response, guardian: r.guardian, answered_at: r.response.answered_at }));

                const allReplies = repliesResult.data || [];
                const uniqueGuardians = new Set(allReplies.filter(r => !r.is_admin_reply).map(r => r.guardian_id));
            ...
            const studentIds = comms
                .filter((c: any) => c.target_type === 'STUDENT' && c.target_ids?.[0])
                .map((c: any) => c.target_ids[0]);
                                    ...
                                    {selectedComm.target_type && (
                                        <>
                                            ...
                                            <span className="flex items-center gap-1.5 px-2 py-0.5 bg-slate-100 text-slate-600 rounded-md text-[10px] font-bold uppercase tracking-tight border border-slate-200">
                                                ...
                                                {selectedComm.target_type === 'SCHOOL' ? 'Escola Toda' : selectedComm.target_type === 'CLASS' ? 'Por Turma' : 'Individual'}
                                            </span>
                                        </>
                                    )}
                                    <span className="text-gray-400 text-sm">•</span>
